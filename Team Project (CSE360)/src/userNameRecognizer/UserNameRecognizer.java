...
public class UserNameRecognizer {
	 ...
	 * @version 1.02		2026-09-01	Updated the FSM design so the first character of the
	 * 									UserName must be alphabetic (UNAlpha) rather than
	 * 									alphanumeric (UNChar), added '&amp;' to the set of
	 * 									separator characters ('-', '_', '.', '&amp;'), and
	 * 									widened the valid length range to 4-32 characters to
	 * 									match the updated FSM diagram
	 *
	 */
	...
	private static boolean running;						// The flag that specifies if the FSM is
														// running
	private static int userNameSize = 0;			// A UserName may not be shorter than 4 nor
	...
	// Private method that answers whether the current character is a UNChar, i.e. one of
	// A-Z, a-z, or 0-9. UNChar is used for every character of a UserName except the very
	// first, which is restricted to UNAlpha (see isUNAlpha below).
	private static boolean isUNChar(char c) {
		return (c >= 'A' && c <= 'Z') ||		// Check for A-Z
				(c >= 'a' && c <= 'z') ||		// Check for a-z
				(c >= '0' && c <= '9');		// Check for 0-9
	}

	// Private method that answers whether the current character is a UNAlpha, i.e. one of
	// A-Z or a-z. UNAlpha is a new, more restrictive terminal class used only for the first
	// character of a UserName, so that a UserName can no longer start with a digit.
	private static boolean isUNAlpha(char c) {
		return (c >= 'A' && c <= 'Z') ||		// Check for A-Z
				(c >= 'a' && c <= 'z');		// Check for a-z
	}

	// Private method that answers whether the current character is one of the separator
	// characters that may appear between two UNChar characters: dash, underscore, period, or
	// (newly added) ampersand.
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0:
				...
				// The current character is checked against A-Z, a-z only (UNAlpha). Unlike the
				// earlier design, a digit is no longer accepted as the first character. If the
				// character is a UNAlpha, the FSM goes to state 1

				// UNAlpha -> State 1
				if (isUNAlpha(currentChar)) {
					nextState = 1;
			...
			case 1:
				...
				//	1: a UNChar (A-Z, a-z, 0-9) that transitions back to state 1
				//  2: a separator ('-', '_', '.', or '&') that transitions to state 2
			...
			case 2:
				// State 2 deals with a character immediately after a separator ('-', '_', '.',
				// or '&') in the name. Only a UNChar may follow a separator; a UserName may not
				// end on a separator nor have two separators in a row.
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage +=
					"A UserName character may only contain the characters A-Z, a-z, 0-9, or " +
					"one of the separators '-', '_', '.', '&' (which may only appear between " +
		...
		case 2:
			...
			userNameRecognizerErrorMessage +=
				"A UserName character immediately after a separator ('-', '_', '.', or '&') " +
				"must be A-Z, a-z, or 0-9. A UserName may not end with a separator, and two " +
				"separators may not appear in a row.\n";
			return userNameRecognizerErrorMessage;
