	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				// The current character may be an alphanumeric character, in which case the FSM
				// stays in state 1 to continue processing the local part of the address
				if ((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
						(currentChar >= '0' && currentChar <= '9')) {	// Digit
					nextState = 1;
				...
				// state 2 to begin processing the domain part of the address
				else if (currentChar == '@') {
					nextState = 2;
					domainPartCounter = 0;		// Reset the domain name part length counter
				}
			...
			case 2: 
				...
				// FSM moves on to state 3 to continue processing the domain part of the address
				if ((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
						(currentChar >= '0' && currentChar <= '9')) {	// Digit
					nextState = 3;
			...
			case 3:
				...
				// The current character may be an alphanumeric character, in which case the FSM
				// stays in state 3 to continue processing this part of the domain name
				if ((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
						(currentChar >= '0' && currentChar <= '9')) {	// Digit
					nextState = 3;
				...
				// The current character may be a period, in which case the FSM moves to state 4
				// to begin processing the next part of the domain name
				else if (currentChar == '.') {
					nextState = 4;
					domainPartCounter = 0;		// Reset the domain name part length counter
				}
				
				// If it is none of those characters, the FSM halts.  State 3 is a final state, so
				// this is only an error if there is still input left to process
				else {
					running = false;
				}
		...
		case 1:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "May only be alphanumeric, a period, or an @ sign.\n";
			return emailAddressErrorMessage + displayInput(input, currentCharNdx);

		case 2:
			// State 2 is not a final state, so we can return a very specific error message
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "The @ sign must be followed by an alphanumeric character.\n";
			return emailAddressErrorMessage + displayInput(input, currentCharNdx);
